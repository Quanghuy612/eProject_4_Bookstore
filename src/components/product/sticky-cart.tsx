...
const StickyCart: React.FC<Props> = ({
  product,
  addToCartLoader,
  handleAddToCart,
  targetButtonRef,
  ...
  return product && isCartVisible ? (
    <div className="w-full z-40 bg-white fixed  left-0 bottom-0 border-t border-border-base py-2 drop-shadow-header">
      <Container>
          ...
          <div className="flex flex-col md:flex-row gap-2 md:gap-5 md:items-center w-full">
            <div>
              <Link
                ...
                className="text-brand-dark font-semibold text-sm leading-5 block mt-1"
              >
                {product.name}
              </Link>
            ...
            <Button
              ...
              className="xs:h-11 xs:text-sm xs:py-3 md:ml-auto"
