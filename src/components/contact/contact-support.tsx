...
const data = [
  ...
  },
  {
    ...
    name: "Open Time",
    description:
      "Our store has re-opened for shopping,</br> exchange Every day 11am to 7pm",
  },
];
  ...
  return (
    <div className="mb-0 3xl:ltr:pr-5 3xl:rtl:pl-5">
      ...
      <div className="mx-auto space-y-5 mb-6">
        {data.map((item) => (
          <div key={`contact--key${item.id}`} className="flex flex-col ">
        ...
        {social && (
          <ul className="flex flex-wrap   space-x-4 md:space-s-5 mx-auto mt-10">
            {social?.map((item) => (
              <li
                className="transition hover:opacity-80"
                key={`social-list--key${item.id}`}
              >
                <Link href={item.path as any} legacyBehavior>
                  <a target="_blank">
                    <Image
                      src={item.image}
                      alt={item.name}
