...
export const CategoriesContainer: FC<blogGridProps> = ({
  categories,
  className = "",
}) => {
        ...
        {filterData?.map((category: any) => {
          ...
          return (
            ...
            >
                ...
                >
                  <div className="flex overflow-hidden   relative z-2">
                    <Image
                      ...
                      width={340}
                      height={210}
                      className=" duration-2000 ease-in-expo hover:scale-109"
                    />
                  </div>
                </Link>
              </div>
              <Link
                href={`${ROUTES.CATEGORY}/${slug}`}
                className={"pt-4  z-10 font-medium leading-5 lg:text-md"}
              >
                {name}
              </Link>
            </article>
