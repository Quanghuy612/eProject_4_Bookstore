...
const DrawerFilter: React.FC<Props> = ({ isDesktop }) => {
  ...
  return (
    <>
      <button
        className={cn(
          "bg-white text-brand-dark w-full flex items-center px-4 py-3 mb-1.5 text-sm font-semibold transition duration-200 ease-in-out  rounded  ",
          isDesktop ? "" : "lg:hidden",
        )}
        onClick={openFilter}
      >
        <FilterIcon />
        <span className="ltr:pl-2.5 rtl:pr-2.5">Product Filters</span>
      </button>
