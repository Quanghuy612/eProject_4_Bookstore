  ...
  return (
    <li className="relative">
      <Link
        href={`${data.path}`}
        className={cn(
          "flex items-center justify-between py-2 ltr:pl-5 rtl:pr-5 xl:ltr:pl-7 xl:rtl:pr-7 ltr:pr-3 rtl:pl-3 xl:ltr:pr-3.5 xl:rtl:pl-3.5  hover:pl-10 ",
        )}
      >
        {data.label}
        {data.subMenu && (
          <span className="text-sm mt-0.5 shrink-0">
            <IoIosArrowForward className="text-body transition duration-300 ease-in-out group-hover:text-brand-dark" />
          </span>
        )}
      </Link>
      {hasSubMenu && (
