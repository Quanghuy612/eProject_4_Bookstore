      ...
      >
        <Link
          ...
          className={cn(
            ...
            {
              [`text-brand-dark font-medium border-b border-border-base ${colorMap[selectedColor].hoverLink}`]:
                depth === 0,
            },
          )}
        >
          ...
          {items && (
            <span className="hidden ltr:ml-auto rtl:mr-auto md:inline-flex">
              <IoIosArrowForward className="text-15px text-brand-dark opacity-40" />
            </span>
          )}
        </Link>
        {Array.isArray(items) ? (
...
function SidebarMenu({ items, className, categoriesLimit }: any) {
  ...
  return (
    <ul
      className={cn(
        "w-full bg-white rounded-b-md category-dropdown-menu ",
        className,
      )}
    >
