        ...
        className={cn(
          " inset-0 m-[15px] md:mt-[30px]  w-full flex  items-center",
          {
            "sm:absolute mx-auto max-w-[480px] md:max-w-[580px] xl:max-w-[700px]":
              variant === "hero",
            "sm:absolute mx-auto max-w-[480px] md:max-w-[580px] xl:max-w-[680px]":
              variant === "hero-3",
        ...
        >
          <p
            className={cn("text-[12px] leading-7 uppercase font-bold ", {
              ...
              "sm:text-sm text-brand-dark dark:text-white":
                variant === "hero-8",
            })}
          >
            {description}
          ...
          <h2
            className={cn("text-2xl md:text-4xl font-semibold mt-2 leading-8", {
              "xl:text-5xl 2xl:text-[36px] text-brand-light leading-snug md:leading-tight xl:leading-[1.3em] mb-3 md:mb-4 xl:mb-3 ":
                variant == "hero" || variant === "hero-4",
              "xs:text-brand-dark dark:text-white 2xl:text-[36px]":
                variant === "hero-3",
              "sm:mt-4 xs:text-brand-dark 2xl:text-[32px] ":
                variant === "hero-6",
              "sm:mt-5 xs:text-brand-dark dark:text-white 2xl:text-[60px] 2xl:leading-[1em] ":
                variant == "hero-8",
            })}
          >
            {title}
          ...
          {banner.btnText && (
            <Link
              href={banner.btnUrl}
              className={cn(
                "text-sm rounded h-[44px] mt-5 md:mt-12 text-base inline-flex items-center justify-center sm:capitalize px-10 py-2 font-semibold ",
                {
                  [`text-brand-dark bg-white  ${colorMap[selectedColor].hoverBg} hover:sm:text-brand-light`]:
                    variant == "hero",
                  [`text-brand-light ${colorMap[selectedColor].bg} hover:text-white hover:bg-brand-dark  `]:
                    variant === "hero-3",
                  [`text-white bg-brand-dark hover:text-white ${colorMap[selectedColor].hoverBg} `]:
                    variant === "hero-6" || variant == "hero-8",
                  "text-brand-dark dark:text-white bg-brand-light  hover:bg-gray-200 dark:hover:bg-brand-light":
                    variant === "hero-4",
                  "xl:min-w-[220px] sm:text-brand-light dark:bg-white ":
                    variant == "hero-8",
                },
              )}
            >
