...
const CollectionDealCard: React.FC<Props> = ({
  ...
  return (
    <Link href={href} className="flex flex-col overflow-hidden group relative">
      <Image
      ...
      <div className=" flex flex-col gap-2  text-center">
        <Heading variant="base" className="pt-4 sm:text-base">
          {title}
        </Heading>
        <p className={cn("text-sm font-medium ")}>{description}</p>
      </div>
    </Link>
  );
};
