      ...
      {categories.map((category) => (
        <div key={category.id} className="space-y-2">
          <div className="flex items-start justify-center ">
            ...
            />
            <div className="pl-2.5   flex-1 flex items-center justify-between">
              <label
                ...
                {category.subCategories.map((subCategory) => (
                  <div key={subCategory.id} className="flex items-start">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        onCategoryChange(subCategory.id, checked)
                      }
                    />
                    <label
                      htmlFor={`category-${subCategory.id}`}
                      className="pl-2.5  text-sm leading-none cursor-pointer group"
                    >
                      <span
                        className={`text-slate-900 dark:text-slate-100 ${colorMap[selectedColor].groupHoverLink}`}
                      >
