...
const BlogPostCard: React.FC<BlogProps> = ({ blogData, className }) => {
  ...
  return (
    <article
      className={cn(
        "flex flex-col product-card overflow-hidden  h-full bg-white rounded-md p-4 xl:p-5",
        className,
      )}
      title={title}
    >
      <div className="pb-5 h-full relative">
        <div
          className={cn(
            "text-base font-semibold mb-2.5 text-skin-primary",
        ...
        <h4
          className={"font-bold text-brand-dark text-2xl lg:text-3xl mb-3.5 "}
        >
          <Link
            href={{ pathname: `${ROUTES.BLOG}/${slug}` }}
            className={cn(
              "text-skin-base line-clamp-2 ",
        ...
        <div className="entry-meta text-13px text-gray-500">
          ...
          <span className="has-dot px-2.5 relative inline-block">{date}</span>
      ...
      <div className="relative mb-10">
        ...
        >
          <div className="card-img-container max-w-[1050px] overflow-hidden flex mx-auto relative rounded">
            <Image
