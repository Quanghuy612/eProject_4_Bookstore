  ...
  const HighLightedBarItems = [
    {
      id: 1,
      title:
        "Updates to Uminex Digital Service Terms of Use and Privacy Policy ",
      href: "/",
    },
    {
      id: 2,
      title: "Welcome to Uminex Join us for 5% off & free delivery ",
      href: "/",
    },
    {
      id: 3,
      title: "Claim your online FREE Delivery or Shipping today",
      href: "/",
    },
  ];
    ...
    <>
      {highlightedBar !== "true" && (
        <HighlightedBar onClose={() => setHighlightedBar("true")}>
          ...
          >
            {HighLightedBarItems.map((item, idx: number) => (
              <SwiperSlide key={`highlightedBar--key-${idx}`}>
                <div className="text-center text-base text-white text-sm">
                  {item.title}
                  <Link
                    className="notification-banner__link underline ml-2 hover:text-blue-500"
                    href="{item.href}"
                  >
                    Learn More
                  </Link>
          ...
          {/* Custom Arrows */}
          <div className="hidden lg:block c-carousel-controls right-0 absolute top-1/2 -translate-y-1/2 mr-12  ">
