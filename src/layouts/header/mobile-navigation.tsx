...
export default function BottomNavigation() {
    ...
    return (
        <>
            <div className="lg:hidden fixed z-30 -bottom-0.5 flex items-center justify-between drop-shadow-mobileNavigation body-font bg-brand-light w-full h-14 px-4 md:px-6 lg:px-8 text-brand-muted pb-0.5">
                <button
                    aria-label="Menu"
                ...
                <AuthMenu href={ROUTES.LOGIN}>
                    <UserIcon />
                </AuthMenu>
            </div>
            <Drawer className="w-[375px]" placement={dir === "rtl" ? "right" : "left"} open={displaySidebar} onClose={closeSidebar} {...motionProps}>
                <MobileMenu />
            </Drawer>
        </>
    );
